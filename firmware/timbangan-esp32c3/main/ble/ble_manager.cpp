...
// ==================== UUID CONSTANTS — harus di atas s_gatt_db ====================
...
static const esp_gatts_attr_db_t s_gatt_db[IDX_NB] = {
    ...
    [IDX_SVC] = {
        ...
        {ESP_UUID_LEN_16,  (uint8_t*)&s_primary_service_uuid,
         ESP_GATT_PERM_READ,
         sizeof(s_service_uuid), sizeof(s_service_uuid),
         s_service_uuid}
    },
    ...
    [IDX_CHAR_DEVICE_INFO] = {
        {ESP_GATT_AUTO_RSP},
        {ESP_UUID_LEN_16, (uint8_t*)&s_char_declare_uuid,
         ESP_GATT_PERM_READ,
         sizeof(uint8_t), sizeof(uint8_t),
         (uint8_t*)&s_char_prop_read}
    },
    [IDX_CHAR_DEVICE_INFO_VAL] = {
        {ESP_GATT_AUTO_RSP},
        {ESP_UUID_LEN_128, s_char_device_info_uuid,
         ESP_GATT_PERM_READ,
    ...
    [IDX_CHAR_WIFI_CFG] = {
        ...
        {ESP_UUID_LEN_16, (uint8_t*)&s_char_declare_uuid,
         ESP_GATT_PERM_READ,
         sizeof(uint8_t), sizeof(uint8_t),
         (uint8_t*)&s_char_prop_write}
    },
    [IDX_CHAR_WIFI_CFG_VAL] = {
        {ESP_GATT_AUTO_RSP},
        {ESP_UUID_LEN_128, s_char_wifi_config_uuid,
         ESP_GATT_PERM_WRITE,
         256, 0, NULL}
    },
    ...
    [IDX_CHAR_DEV_CFG_VAL] = {
        {ESP_GATT_AUTO_RSP},
        {ESP_UUID_LEN_128, s_char_device_config_uuid,
         ESP_GATT_PERM_WRITE,
         256, 0, NULL}
    },

    // --- PROV STATUS (READ + NOTIFY) ---
    [IDX_CHAR_PROV_STATUS] = {
        {ESP_GATT_AUTO_RSP},
        {ESP_UUID_LEN_16, (uint8_t*)&s_char_declare_uuid,
         ESP_GATT_PERM_READ,
         sizeof(uint8_t), sizeof(uint8_t),
         (uint8_t*)&s_char_prop_read_notify}
    },
    [IDX_CHAR_PROV_STATUS_VAL] = {
        {ESP_GATT_AUTO_RSP},
        {ESP_UUID_LEN_128, s_char_prov_status_uuid,
         ESP_GATT_PERM_READ,
...
static void notify_status(const char* status, const char* ip, const char* error) {
    ...
    esp_ble_gatts_send_indicate(
        ...
        strlen(s_prov_status_val),
        (uint8_t*)s_prov_status_val,
        false
    );
...
static void start_advertising(void) {
    ...
    char adv_name[48];
    snprintf(adv_name, sizeof(adv_name),
...
static void handle_write(uint16_t handle, uint8_t* data, uint16_t len) {
    ...
    if (handle == s_handle_table[IDX_CHAR_WIFI_CFG_VAL]) {
        ...
        if (json) {
            ...
            cJSON_Delete(json);
            LOG_I(TAG, "WiFi SSID: %s", s_prov_config.ssid);
        }
    }
        ...
        if (json) {
            ...
            cJSON_Delete(json);
            LOG_I(TAG, "Device name: %s", s_prov_config.device_name);

            // Simpan ke NVS jika WiFi sudah ada
            if (strlen(s_prov_config.ssid) > 0 &&
                ...
                if (nvs_open(NVS_NAMESPACE, NVS_READWRITE, &nvs) == ESP_OK) {
                    ...
                    int32_t port = s_prov_config.backend_port > 0 ?
                                   s_prov_config.backend_port : 8000;
    ...
    switch (event) {
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:
            if (param->add_attr_tab.status == ESP_GATT_OK &&
                ...
                LOG_E(TAG, "Create attr table failed: %d handles",
                      param->add_attr_tab.num_handle);
            }
            break;

        case ESP_GATTS_START_EVT:
            LOG_I(TAG, "Service started");

            // Populate device info saat service start
            {
                uint8_t mac[6];
                ...
                snprintf(s_device_info_val, sizeof(s_device_info_val),
                    "{\"mac\":\"%s\",\"type\":\"%s\","
                    "\"firmware\":\"%s\",\"chip\":\"%s\",\"name\":\"\"}",
                    mac_str, DEVICE_TYPE, FIRMWARE_VERSION, DEVICE_CHIP);

                // Update attribute value di GATT server
        ...
        case ESP_GATTS_DISCONNECT_EVT:
            ...
            LOG_I(TAG, "Client disconnected (reason: 0x%x)",
        ...
        case ESP_GATTS_READ_EVT: {
            if (param->read.handle == s_handle_table[IDX_CHAR_DEVICE_INFO_VAL]) {
                uint8_t mac[6];
                esp_read_mac(mac, ESP_MAC_WIFI_STA);
                char mac_str[18];
                snprintf(mac_str, sizeof(mac_str),
...
void ble_manager_init(void) {
    // ESP32-C3 controller-nya BLE-only — tidak ada memori Classic BT untuk dilepas.
    // Panggilan ini hanya relevan di ESP32 klasik.
...
void ble_manager_start_provisioning(void) {
    if (s_state == BLE_STATE_ADVERTISING ||
        s_state == BLE_STATE_CONNECTED) {
        LOG_W(TAG, "Already provisioning");
        return;
    }
    ...
    LOG_I(TAG, "BLE provisioning started — timeout: %d ms", BLE_PROV_TIMEOUT_MS);
}

void ble_manager_shutdown(void) {
    LOG_I(TAG, "Shutting down BLE...");
    esp_ble_gap_stop_advertising();
    ...
    LOG_I(TAG, "BLE shutdown complete");
}
