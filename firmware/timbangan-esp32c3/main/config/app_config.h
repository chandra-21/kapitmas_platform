...
// ==================== FIRMWARE ====================
#define FIRMWARE_VERSION        "v4.0.0-esp32c3"
#define FIRMWARE_BUILD          __DATE__ " " __TIME__
#define DEVICE_CHIP             "ESP32-C3"

// ==================== HARDWARE PINS — ESP32-C3 SUPER MINI ====================
// GPIO yang tersedia di board: 0-10, 20, 21
//   GPIO8  = LED biru onboard (active LOW, ada pull-up ke 3V3)
//   GPIO9  = tombol BOOT onboard (active LOW, ada pull-up)
//   GPIO18 / GPIO19 = USB D-/D+ (native USB) — JANGAN dipakai
//   GPIO11-17 = flash internal — JANGAN dipakai
//   GPIO20 / GPIO21 = pin UART0 (silkscreen RX/TX); bebas dipakai karena
//                     console dialihkan ke USB Serial/JTAG (lihat sdkconfig.defaults)
#define PIN_LED1                8      // status sistem — LED onboard
#define PIN_LED2                10     // indikator data — LED eksternal
#define PIN_BOOT                9      // tombol BOOT onboard
#define PIN_UART1_RX            20     // ← TX timbangan
#define PIN_UART1_TX            21     // → RX timbangan (tidak dipakai firmware ini)

// Polaritas LED — LED onboard C3 Super Mini nyala saat pin LOW
#define LED1_ACTIVE_LOW         1
#define LED2_ACTIVE_LOW         0

#define LED1_LEVEL(on)          (LED1_ACTIVE_LOW ? !(on) : (on))
#define LED2_LEVEL(on)          (LED2_ACTIVE_LOW ? !(on) : (on))
...
// Scan diagnostik saat boot — cetak RSSI/channel/authmode semua AP terlihat.
// Menambah ~2-3 detik waktu boot, jadi dimatikan untuk pemakaian normal.
// Set 1 saat menyelidiki masalah asosiasi WiFi di lokasi pemasangan.
#define WIFI_SCAN_DIAGNOSTIC        0
...
// ==================== SERIAL (UART1) ====================
// ESP32-C3 hanya punya UART0 dan UART1 — UART2 tidak ada.
// UART0 dipakai ROM bootloader, jadi timbangan dibaca lewat UART1.
#define UART1_BAUD_RATE             9600
#define UART1_BUFFER_SIZE           256
#define UART1_TIMEOUT_MS            500
#define UART1_MIN_INTERVAL_MS       100
#define UART1_TASK_STACK            4096
#define UART1_TASK_PRIORITY         4
