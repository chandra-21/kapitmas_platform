...
static void serial_task(void* arg) {
    uint8_t  buf[UART1_BUFFER_SIZE];
    char     line[UART1_BUFFER_SIZE];
    int      idx          = 0;
    ...
    while (true) {
        ...
        for (int i = 0; i < len; i++) {
                ...
                if (idx > 0) {
                    ...
                    if (weight > ZERO_THRESHOLD) {
                        ...
                        if (!s_waiting_for_zero &&
                            (now_ms - last_read_ms) >= UART1_MIN_INTERVAL_MS) {
            ...
            } else if (idx < UART1_BUFFER_SIZE - 1) {
                line[idx++] = c;
            } else {
...
void serial_reader_init(void) {
    ...
    uart_cfg.baud_rate  = UART1_BAUD_RATE;
    ...
    ESP_ERROR_CHECK(uart_driver_install(UART_NUM_1,
                                        UART1_BUFFER_SIZE * 2, 0, 0, NULL, 0));
    ...
    LOG_I(TAG, "UART1 initialized - baud: %d | RX: %d | TX: %d",
          UART1_BAUD_RATE, PIN_UART1_RX, PIN_UART1_TX);
}
